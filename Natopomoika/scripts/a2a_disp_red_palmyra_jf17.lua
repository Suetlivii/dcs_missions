...
if ME_isRedCap == true then
    ...
    redA2ADispatcher:SetSquadron( "jf17_palmyra", AIRBASE.Syria.Palmyra, { "cap_jf17_palmyra" }, 4 )
    redA2ADispatcher:SetSquadronTakeoffFromParkingCold( "jf17_palmyra" )
    redA2ADispatcher:SetSquadronLandingAtRunway( "jf17_palmyra" )
    redA2ADispatcher:SetSquadronFuelThreshold( "jf17_palmyra", 0.30 )
    redA2ADispatcher:SetSquadronGrouping( "jf17_palmyra", 2 )
    redA2ADispatcher:SetSquadronOverhead( "jf17_palmyra", 1 )

    if math.random() <= red_gci_chance then
        redA2ADispatcher:SetSquadronGci( "jf17_palmyra", 750, 1100 )
    end
    ...
    if math.random() <= red_cap_chance then
        ...
        redA2ADispatcher:SetSquadronCap( "jf17_palmyra", redA2ACapZone, 5000, 9000, 600, 750, 800, 1100, "RADIO" )
        --name, max spawned groups, min time sec, max time sec
        redA2ADispatcher:SetSquadronCapInterval( "jf17_palmyra", 1, red_cap_interval_min, red_cap_interval_max, 1 )
    end
end