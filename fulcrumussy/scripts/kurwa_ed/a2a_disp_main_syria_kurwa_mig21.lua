...
-- in minutes
local blue_cap_interval_min = 40 * 60
local blue_cap_interval_max = 60 * 60
...
if ME_isBlueCap == true then
    ...
    BlueA2ADispatcher:SetGciRadius( 85000 )
    BlueA2ADispatcher:SetDisengageRadius(140000)
    BlueA2ADispatcher:SetTacticalDisplay( ME_isDebugMode )
    local BlueA2ACapZone = ZONE:New("blue_cap_zone_mig21")

    --https://flightcontrol-master.github.io/MOOSE_DOCS/Documentation/Wrapper.Airbase.html
    BlueA2ADispatcher:SetSquadron( "fsa_mig21_1st", AIRBASE.Syria.Tiyas, { "fsa_mig21" }, 6 )
    BlueA2ADispatcher:SetSquadronTakeoffFromParkingCold( "fsa_mig21_1st" )
    BlueA2ADispatcher:SetSquadronLandingAtRunway( "fsa_mig21_1st" )
    BlueA2ADispatcher:SetSquadronFuelThreshold( "fsa_mig21_1st", 0.30 )
    BlueA2ADispatcher:SetSquadronGrouping( "fsa_mig21_1st", 2 )
    BlueA2ADispatcher:SetSquadronOverhead( "fsa_mig21_1st", 1 )

    if math.random() <= blue_gci_chance then
        BlueA2ADispatcher:SetSquadronGci( "fsa_mig21_1st", 750, 1100 )
    end
    ...
    if math.random() <= blue_cap_chance then
        ...
        BlueA2ADispatcher:SetSquadronCap( "fsa_mig21_1st", BlueA2ACapZone, 5000, 9000, 600, 750, 800, 1100, "RADIO" )
        --name, max spawned groups, min time sec, max time sec
        BlueA2ADispatcher:SetSquadronCapInterval( "fsa_mig21_1st", 1, blue_cap_interval_min, blue_cap_interval_max, 1 )
    end
end